...
It's a VGG-like network [2] with many 3x3 filters and padding 1,1 so the sizes of a feature map after then are not changed. They are only changed after max-pooling.

We will initialize the weights MSR-style [3]:
