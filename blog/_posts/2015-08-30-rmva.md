...
reproduces the MNIST results of the paper. We are still working on 
...
Here are some glimpse sequences (the first frames start at the same location) :

![locations](images/output_8PHJ4z.gif)

And this is what the model sees for those sequences:

![glimpses](images/output_Onbzrk.gif)

So basically, REINFORCE did a good job of teaching the module to focus 
its attention on different regions. A failure mode would have been to 
see the model do this :

![fail](images/output_6kgOs1.gif)

In that case, the `opt.unitPixels` was set to 6 instead of 12 (so the attention was stuck in the middle).
...
![translatedmnist](images/output_52iFki.gif)

For this dataset, the images are `60x60` where each image contains a randomly placed `28x28` MNIST digit.
The glimpse uses a depth of 3 scales, where each successive patch is twice the height and width of the previous one.
We have only started training with this dataset a couple of days ago.
