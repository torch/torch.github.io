...
1. The location sensor. Its input is `x`, `y` coordinates of the current glimpse location, 
...
2. The glimpse sensor is *what* its looking at :
```lua
...
3. The glimpse network is where the glimpse and location sensor mix through 
a Multi-Layer Perceptron (MLP) to form the input layer of the Recurrent Neural Network (RNN) :
```lua
...
5. We have already seem the locator network above. 
It is responsible to sample the next location `a[t]` given the previous hidden state `h[t-t]`.
We should however bring your attention to the `opt.unitPixels` variable, 
as it is very important. This basically, outlines how far near the borders the 
center of each glimpse can reach. Specifically, since the 
...
6. We wanted to make all this work nice with the `nn` package, so we decided 
to implement a module that took the full image as input, and output the sequence of 
hidden states `h`. As its name implies, this module is a general purpose 
...
7. The agent adds a classifier to the `RecurrentAttention` module.
The classifier's input is the last hidden state `h[T]` where `T` is the number of time-steps (i.e. glimpses to take):
...
8. You might recall that the REINFORCE algorithm requires a baseline reward `b`?
...
The `agent` instance is the RAM model detailed in the paper. 
